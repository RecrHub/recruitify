...
import { JwtResponse, RegisterRequest, MessageResponse, RefreshTokenRequest, LoginRequest } from '@/types/auth';
...
const authService = {
  ...
  login: async (email: string, password: string): Promise<JwtResponse> => {
    try {
      ...
      const response = await authAxios.post<JwtResponse>('/api/v1/auth/login', loginRequest);
  ...
  register: async (registerData: RegisterRequest): Promise<MessageResponse> => {
    const response = await authAxios.post<MessageResponse>('/api/v1/auth/register', registerData);
    return response.data;
  },
